module.exports = function(io) {
    ...
    io.on("connection", function (socket) {socket.join("room")
        ...
        socket.on('getTemp', function () {
            ...
            console.log(" NODE sending Temp_API -> " + ROOM_ID_STAT.tempSet)
        })
        ...
        socket.on('setTemp', function (data) {
           // console.log( 'TRY setTemp -> ' + data.setTemp +' : '+ ROOM_ID.cool_temp_min+' / '+ROOM_ID.cool_temp_max);
            ...
            socket.broadcast.emit("Temp_API", ROOM_ID_STAT.tempSet )   // broadcast to all open apps

            if(ROOM_ID_STAT.fanSet === 1){     // OFF_LINE
                ROOM_ID_STAT.fanSet = 2;
                io.emit("Fan_API", ROOM_ID_STAT.fanSet )     // EMIT --> send to app
                socket.broadcast.emit("Fan_API", ROOM_ID_STAT.fanSet )   //  broadcast to all open apps
