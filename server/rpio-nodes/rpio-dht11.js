 ...
 */
const tmp = new Array(buf.length); /* 0.8 compat */
 ...
 */
const rh_int = parseInt(data.slice(0, 8).join(''), 2);
const rh_dec = parseInt(data.slice(8, 16).join(''), 2);
const tm_int = parseInt(data.slice(16, 24).join(''), 2);
const tm_dec = parseInt(data.slice(24, 32).join(''), 2);
const chksum = parseInt(data.slice(32, 40).join(''), 2);

/* chksum should match this calculation */
const chk = ((rh_int + rh_dec + tm_int + tm_dec) & 0xFF);
